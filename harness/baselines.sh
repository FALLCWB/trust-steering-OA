#!/usr/bin/env bash
set -u
H=/opt/trust-lab/harness; DATA=/opt/trust-lab/data/steady_baselines; mkdir -p "$DATA"
export TARPIT_DELAY_MS=0
BASE=8; ADUR=75; COOL=4; REPS=5; INTENS=2000
declare -A CFG=( [droponly]=droponly.json [binary]=binary.json )
...
"$H/labctl.sh" topo-stop>/dev/null 2>&1; echo "BASELINES DONE $(date)"

...
CSV = os.environ.get("OVH_CSV",
                     os.environ.get("TLAB_DATA", "/opt/trust-lab/data") + "/overhead_attack/scaling.csv")
...
SPOOF_PPS = 931  # fresh-source-per-packet Packet-In rate: randomized-source experiment, n=10, 931+-2 (rev2 spoof_ip_random)
...
# installed rules (controller FLOW_MODs) and switch flow-table occupancy coincide exactly:
# every installed rule stays resident (no eviction during the run), so one line carries both.
if flow_mod == ovs_flows:
    ax.plot(N, flow_mod, "o-", color="#1f77b4", ms=5, lw=1.8,
            label="installed rules = switch flow-table")
else:
    ax.plot(N, flow_mod, "o-", color="#1f77b4", ms=5, lw=1.8, label="installed rules")
    ax.plot(N, ovs_flows, "D-", color="#2ca02c", ms=4, mfc="none", label="switch flow-table")
ax.plot(N, pkt_in, "s--", color="#d62728", ms=4, label="Packet-In events")
ax.plot(N, N, ":", color="#999", lw=1, label="slope-1 reference")
